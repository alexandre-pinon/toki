...
export const getShoppingListItems = async (userId: string): Promise<ShoppingListSection[]> => {
...
export const addShoppingListItem = async (item: Omit<ShoppingItem, "id">) => {
...
export const updateShoppingListItem = async (id: string, item: Omit<ShoppingItem, "id">) => {
  getDbResponseDataOrThrow(
    await supabase
      .from("shopping_items")
      .update({
...
export const setCheckedShoppingListItems = async (ids: string[], checked: boolean) => {
  if (ids.length === 0) {
    return;
  }
